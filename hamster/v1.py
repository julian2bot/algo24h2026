...
def simulate(grid):
    ...
    for _ in range(2000):
        ...
        vis = visible_light(x, y, lights, blocks)

        if len(vis) >= 2:
            return steps

        if len(vis) == 1:
            dx, dy = -vis[0][0], -vis[0][1]
...
if __name__ == "__main__":
    ...
    with open(f"{N}_challenge1.txt", "w") as f:
        f.write(txt)

    print("EXPORTED:", f"{N}_challenge1.txt")
